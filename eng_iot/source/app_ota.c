...
struct ota_control_stu ota_control;
...
extern void Data_Print(char *string, uint8_t *buff, uint16_t len);
int ota_data_check()
{
		unsigned int CRC32 = 0xFFFFFFFF;
		uint8_t *data;
	...
	//		printf("offset:%d,CRC32:%0x\r\n",offset,CRC32);
			total_len -= read_len;
			offset += read_len;
			
		}
		if(CRC32 == ota_config.file_crc32) {
			ota_config.magic = MAGIC;
			ota_config.ota_flag = 1;
			ota_config.config_crc32 = GetCrc32((uint8_t *)&ota_config, sizeof(ota_config) - 4);
			ota_flash_write(BACK_APP_CONFIG_ADR, &ota_config, sizeof(ota_config));
			free(data);
			return 0;
		} else {
			printf("dataCRC32:%0x, file_crc32:%0x", CRC32, ota_config.file_crc32);
			free(data);
			return -1;
		}
}
